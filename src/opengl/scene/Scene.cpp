...
{
    m_lights.SetDirectionalLight(light);
}
...
LightEnvironment &Scene::GetLightEnvironment()
{
    return m_lights;
}
                                          ...
                                          float orbitTargetYOffset)
{
    ...
    if (cam.GetMode() == CameraMode::ThirdPerson)
    {
        ...
        if (scrollDelta != 0.0f)
        {
            ...
            if (m_cameraThirdPersonSpeed < 0.1f)
                m_cameraThirdPersonSpeed = 0.1f;
            if (m_cameraThirdPersonSpeed > 100.0f)
                m_cameraThirdPersonSpeed = 100.0f;
    ...
    if (cam.GetMode() == CameraMode::FreeFly)
    {
        ...
        if (fwd > 0.0f)
            cam.Move(CameraDirection::Forward, currentFreeFly, deltaTime);
        if (fwd < 0.0f)
            cam.Move(CameraDirection::Backward, currentFreeFly, deltaTime);
        if (right > 0.0f)
            cam.Move(CameraDirection::Right, currentFreeFly, deltaTime);
        if (right < 0.0f)
            cam.Move(CameraDirection::Left, currentFreeFly, deltaTime);
        if (up > 0.0f)
            cam.Move(CameraDirection::Up, currentFreeFly, deltaTime);
        if (up < 0.0f)
            cam.Move(CameraDirection::Down, currentFreeFly, deltaTime);
    }
    else
    {

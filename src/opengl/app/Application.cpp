...
bool Application::Initialize() {
    ...
    for (const auto &v: versions) {
        ...
        m_window = glfwCreateWindow(options.window.width, options.window.height, options.window.title.c_str(), nullptr,
                                    nullptr);
        if (m_window) {
            windowCreated = true;
    ...
    if (!windowCreated) {
        spdlog::error(
            "[Application] Failed to create GLFW window — no supported OpenGL context available (tried 4.6 down to 3.3 Core Profile)");
        glfwTerminate();
        return false;
    }
...
void Application::Update(Scene &scene) {
    ...
    if (m_imguiInitialized) {
        ...
        ImGuiWindowFlags topbarFlags = ImGuiWindowFlags_NoDecoration |
                                       ImGuiWindowFlags_NoMove |
                                       ImGuiWindowFlags_NoScrollbar |
                                       ImGuiWindowFlags_NoSavedSettings;
        if (lookModeActive) topbarFlags |= ImGuiWindowFlags_NoMouseInputs;

        ImGui::SetNextWindowPos(ImVec2(0, 0), ImGuiCond_Always);
        ImGui::SetNextWindowSize(ImVec2(static_cast<float>(w), topbarHeight), ImGuiCond_Always);
        ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(10, 8));
        ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(15, 4));
        ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0.08f, 0.08f, 0.12f, 0.95f));

        if (ImGui::Begin("##Topbar", nullptr, topbarFlags)) {
            // Left side: Project title
            ImGui::Text("Forward Renderer");
            ImGui::SameLine(0, 30);
            ImGui::TextDisabled("|");
            ImGui::SameLine();
            ImGui::Text("Scene: %s", scene.GetName().empty() ? "Untitled" : scene.GetName().c_str());

            // Right side: FPS and help button
            const float rightAreaWidth = 200.0f;
            ImGui::SameLine(ImGui::GetWindowWidth() - rightAreaWidth);
            ImGui::Text("%.1f FPS | %.2f ms", stats.fps, stats.frameTimeMs);
            ImGui::SameLine();
        ...
        // ═══════════════════════════════════════════════════════════════
        // HELP WINDOW (modal-style, centered)
        // ═══════════════════════════════════════════════════════════════
        if (m_showHelpWindow) {
            ...
            if (ImGui::Begin("Application Guide", &m_showHelpWindow, guideFlags)) {
                ImGui::TextWrapped(
                    "Welcome! Use this guide to navigate through the application and interact with the UI.");
                ImGui::Spacing();
                ...
                ImGui::BulletText(
                    "1-4: Switch between different project scenes (1: Sample, 2: Solar System, 3: Diorama, 4: Neon City).");
                ImGui::Spacing();
                ImGui::SeparatorText("Mouse Controls");
                ImGui::BulletText(
                    "Mouse Movement: Look around (only active when the mouse cursor is captured via TAB).");
                ...
                ImGui::TextWrapped(
                    "Press TAB to release the mouse cursor and interact with this UI. On the left, you can use the sidebar to add and manipulate point lights, check renderer performance, and change shadow parameters. Changes apply in real-time!");
            }
            ImGui::End();
        }
        ...
        const float sidebarTop = topbarHeight;
        const float sidebarHeight = static_cast<float>(h) - topbarHeight;

        ImGuiWindowFlags sidebarFlags = ImGuiWindowFlags_NoDecoration |
                                        ImGuiWindowFlags_NoMove |
                                        ImGuiWindowFlags_NoSavedSettings;
        if (lookModeActive) sidebarFlags |= ImGuiWindowFlags_NoMouseInputs;

        ImGui::SetNextWindowPos(ImVec2(0, sidebarTop), ImGuiCond_Always);
        ImGui::SetNextWindowSize(ImVec2(sidebarWidth, sidebarHeight), ImGuiCond_Always);
        ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(12, 12));
        ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0.10f, 0.10f, 0.14f, 0.92f));
        ...
        if (ImGui::Begin("##Sidebar", nullptr, sidebarFlags)) {
            ...
            if (ImGui::CollapsingHeader("Camera", ImGuiTreeNodeFlags_DefaultOpen)) {
                if (submission.camera) {
                    ...
                    switch (submission.camera->GetMode()) {
                        case CameraMode::FirstPerson: modeStr = "FirstPerson";
                            break;
                        case CameraMode::ThirdPerson: modeStr = "ThirdPerson";
                            break;
                        default: break;
                    }
            ...
            if (ImGui::CollapsingHeader("Renderer Stats", ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                if (stats.approxTriangleCountApproximate)
                    ImGui::TextDisabled(
                        "Triangle estimate uses index/vertex counts and treats non-triangle topologies as 0.");
            }
            ...
            if (ImGui::CollapsingHeader("Active Lights", ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                ImGui::TextDisabled(
                    "Interactive: edits write back to the current scene lights and apply on the next frame.");
            ...
            if (ImGui::CollapsingHeader("Shadow Debug")) {
                ...
                ImGui::TextDisabled(
                    "Interactive: edits write back to per-light shadow config and apply on the next frame.");
                ImGui::Spacing();
                ...
                if (stats.shadowPassDataAvailable && stats.shadowPassObjectCount == 0)
                    ImGui::TextDisabled(
                        "Live directional shadow pass ran, but no submitted object reached the depth draw path.");
                if (!stats.shadowPassDataAvailable)
                    ImGui::TextDisabled(
                        "Directional shadow pass is inactive for this frame, so pass-only counters stay at 0.");
                ...
                if (stats.shadowMapPreviewAvailable) {
                    ...
                    const float previewHeight = stats.shadowMapWidth > 0
                                                    ? (previewWidth * static_cast<float>(stats.shadowMapHeight) /
                                                       static_cast<float>(stats.shadowMapWidth))
                                                    : previewWidth;
                ...
                if (liveLights.GetPointLights().empty()) {
                    ImGui::TextDisabled("No point-light shadow config in the active scene.");
                } else {
                    for (std::size_t i = 0; i < liveLights.GetPointLights().size(); ++i) {
                        const std::string label = "Point Shadow " + std::to_string(i + 1);
                        ImGui::PushID(static_cast<int>(i));
                        if (ImGui::TreeNode(label.c_str())) {
                            ImGui::Text("Name      : %s",
                                        liveLights.GetPointLights()[i].name.empty()
                                            ? "(unnamed)"
                                            : liveLights.GetPointLights()[i].name.c_str());
                            DrawShadowParamsDebug(
                                liveLights.GetPointLights()[i].shadow,
                                "Far plane",
                ...
                if (!liveLights.GetSpotLights().empty())
                    ImGui::TextDisabled(
                        "Spot-light shadow UI is not added in this phase; shadow-enabled count is shown above.");
            }

            if (ImGui::CollapsingHeader("Resource Cache")) {
